...
import braces.gui.dialog.CreateEmployeeDialog;
import braces.gui.dialog.EditEmployeeDialog;
...
public class EmployeePage extends javax.swing.JPanel {
    ...
    public void init() {
        ...
        String[] searchType = {"Tất cả", "Mã", "Tên", "Vai trò", "Sđt", "Trạng thái", "Email"};
    ...
    public void loadTable() {
        ...
        for (TaiKhoan tk : list) {
            Object[] row = {tk.getManv(), tk.getTenNv(), tk.getUsername(), ct.getRoleText(tk.getRole()),
                tk.getSoDienThoai(), ct.getTrangThaiText(tk.getTrangThai()),
                tk.getEmail()};
            model.addRow(row);
        }
    }
    ...
    public void edit(int row) {
        ...
        Optional<TaiKhoan> tk = ct.getById(id);
        Dialog dialog = new EditEmployeeDialog(null, true, this, tk.get());
        dialog.setLocationRelativeTo(null);
        dialog.setVisible(true);
    }
    public void info(int row) {
        int id = (int) table.getValueAt(row, 0);
        Optional<TaiKhoan> tk = ct.getById(id);
        Dialog dialog = new InfoEmployeeDialog(null, true, tk.get());
        dialog.setLocationRelativeTo(null);
        dialog.setVisible(true);
    }
    ...
    private void initComponents() {
        ...
        add(headerPanel2, java.awt.BorderLayout.NORTH);
        ...
        table.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Mã nhân viên", "Tên", "Tài khoản", "Vai trò", "Số điện thoại", "Trang thái", "Email"
            }
        ) {
    ...
    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
        Dialog dialog = new CreateEmployeeDialog(null, true, this);
        dialog.setLocationRelativeTo(null);
        dialog.setVisible(true);
    ...
    private void btnDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteActionPerformed
        try {
            int row = table.getSelectedRow();
              if (row == -1) {
            MessageDialog.error(this, "Chưa chọn dòng để xoá!");
            return;
        }
              if (Objects.equals(XAuth.taikhoan.getManv(), list.get(row).getManv())) {
                  MessageDialog.error(this, "Không thể xoá chính bạn!");
                  return;
              }
            delete(row);
        } catch (Exception e) {
            MessageDialog.error(this, "Không thể xoá");
        }
